...
module.exports = function (client) {
    client.on("message", async message => {
        if (message.content.startsWith(config.bot.prefix + "tops rx")) {

            try {
                var recentScore = await query("SELECT * FROM scores_relax INNER JOIN users ON scores_relax.userid = users.id WHERE privileges > 2 ORDER BY pp DESC;")
                var mapid = await query("SELECT * FROM beatmaps WHERE beatmap_md5 = ?", recentScore[0].beatmap_md5)
                let color = randomcolor_1.randomColor();
                let hex = parseInt(color.replace(/^#/, ''), 16);

                const embed = new Discord.RichEmbed()
                    .setTitle("Server top relax play set by " + recentScore[0].username)
